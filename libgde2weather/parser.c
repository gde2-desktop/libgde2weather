...
/**
 * gde2weather_parser_get_value:
 * @parser: a #Gde2WeatherParser
 *
 ...
 **/
char *
gde2weather_parser_get_value (Gde2WeatherParser *parser)
{
    char *value;
...
/**
 * gde2weather_parser_get_localized_value:
 * @parser: a #Gde2WeatherParser
 *
 ...
 **/
char *
gde2weather_parser_get_localized_value (Gde2WeatherParser *parser)
{
...
Gde2WeatherParser *
gde2weather_parser_new (gboolean use_regions)
{
    ...
    parser = g_slice_new0 (Gde2WeatherParser);
     ...
     */
    if (!filename)
	filename = g_build_filename (GDE2WEATHER_XML_LOCATION_DIR, "Locations.xml", NULL);

    if (!g_file_test (filename, G_FILE_TEST_IS_REGULAR) && zlib_support) {
        g_free (filename);
	filename = g_build_filename (GDE2WEATHER_XML_LOCATION_DIR, "Locations.xml.gz", NULL);
    }
    ...
    keep_going = tagname && !strcmp (tagname, "gde2weather");
    xmlFree (tagname);
...
void
gde2weather_parser_free (Gde2WeatherParser *parser)
{
    if (parser->xml)
	xmlFreeTextReader (parser->xml);
    g_slice_free (Gde2WeatherParser, parser);
}
