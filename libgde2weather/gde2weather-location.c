...
/* gde2weather-location.c - Location-handling code
 *
...
#include "gde2weather-location.h"
#include "gde2weather-timezone.h"
#include "parser.h"
#include "weather-priv.h"

/**
 * SECTION:gde2weather-location
 * @Title: Gde2WeatherLocation
 *
 * A #Gde2WeatherLocation represents a "location" of some type known to
 * libgde2weather; anything from a single weather station to the entire
 * world. See #Gde2WeatherLocationLevel for information about how the
 * hierarchy of locations works. 
 */

struct _Gde2WeatherLocation {
    char *name, *sort_name;
    Gde2WeatherLocation *parent, **children;
 ...
 * @GDE2WEATHER_LOCATION_WORLD: A location representing the entire world.
 * @GDE2WEATHER_LOCATION_REGION: A location representing a continent or
 * other top-level region.
 * @GDE2WEATHER_LOCATION_COUNTRY: A location representing a "country" (or
 * other geographic unit that has an ISO-3166 country code)
 * @GDE2WEATHER_LOCATION_ADM1: A location representing a "first-level
 * administrative division"; ie, a state, province, or similar
 * division.
 * @GDE2WEATHER_LOCATION_ADM2: A location representing a subdivision of a
 * %GDE2WEATHER_LOCATION_ADM1 location. (Not currently used.)
 * @GDE2WEATHER_LOCATION_CITY: A location representing a city
 * @GDE2WEATHER_LOCATION_WEATHER_STATION: A location representing a
 * weather station.
 *
 * The size/scope of a particular #Gde2WeatherLocation.
 *
 * Locations form a hierarchy, with a %GDE2WEATHER_LOCATION_WORLD
...
{
    Gde2WeatherLocation *loc_a = *(Gde2WeatherLocation **)a;
    Gde2WeatherLocation *loc_b = *(Gde2WeatherLocation **)b;
...
sort_locations_by_distance (gconstpointer a, gconstpointer b, gpointer user_data)
{
    Gde2WeatherLocation *loc_a = *(Gde2WeatherLocation **)a;
    Gde2WeatherLocation *loc_b = *(Gde2WeatherLocation **)b;
    Gde2WeatherLocation *city = (Gde2WeatherLocation *)user_data;
    double dist_a, dist_b;

    dist_a = gde2weather_location_get_distance (loc_a, city);
    dist_b = gde2weather_location_get_distance (loc_b, city);
    if (dist_a < dist_b)
	return -1;
...
static Gde2WeatherLocation *
location_new_from_xml (Gde2WeatherParser *parser, Gde2WeatherLocationLevel level,
		       Gde2WeatherLocation *parent)
{
    Gde2WeatherLocation *loc, *child;
    GPtrArray *children = NULL;
    const char *tagname;
    char *value, *normalized;
    int tagtype, i;

    loc = g_slice_new0 (Gde2WeatherLocation);
    loc->parent = parent;
    loc->level = level;
    loc->ref_count = 1;
	...
	if (!strcmp (tagname, "name") && !loc->name) {
	    value = gde2weather_parser_get_localized_value (parser);
	    if (!value)
		goto error_out;
	    ...
	    value = gde2weather_parser_get_value (parser);
	    if (!value)
		goto error_out;
	    ...
	    child = location_new_from_xml (parser, GDE2WEATHER_LOCATION_REGION, loc);
	    if (!child)
		goto error_out;
		...
		if (child->children) {
			...
			g_ptr_array_add (children, gde2weather_location_ref (child->children[i]));
		}
		gde2weather_location_unref (child);
	    }
	} else if (!strcmp (tagname, "country")) {
	    child = location_new_from_xml (parser, GDE2WEATHER_LOCATION_COUNTRY, loc);
	    if (!child)
		goto error_out;
	    g_ptr_array_add (children, child);
	} else if (!strcmp (tagname, "state")) {
	    child = location_new_from_xml (parser, GDE2WEATHER_LOCATION_ADM1, loc);
	    if (!child)
		goto error_out;
	    g_ptr_array_add (children, child);
	} else if (!strcmp (tagname, "city")) {
	    child = location_new_from_xml (parser, GDE2WEATHER_LOCATION_CITY, loc);
	    if (!child)
		goto error_out;
	    g_ptr_array_add (children, child);
	} else if (!strcmp (tagname, "location")) {
	    child = location_new_from_xml (parser, GDE2WEATHER_LOCATION_WEATHER_STATION, loc);
	    if (!child)
		goto error_out;
	    g_ptr_array_add (children, child);

	} else if (!strcmp (tagname, "timezones")) {
	    loc->zones = gde2weather_timezones_parse_xml (parser);
	    if (!loc->zones)
		goto error_out;
    ...
    if (children->len) {
	if (level == GDE2WEATHER_LOCATION_CITY)
	...
	loc->children = (Gde2WeatherLocation **)g_ptr_array_free (children, FALSE);
    } else
...
error_out:
	...
	gde2weather_location_unref (children->pdata[i]);
    g_ptr_array_free (children, TRUE);
...
/**
 * gde2weather_location_new_world:
 * @use_regions: whether or not to divide the world into regions
 *
 * Creates a new #Gde2WeatherLocation of type %GDE2WEATHER_LOCATION_WORLD,
 ...
 * location will be %GDE2WEATHER_LOCATION_REGION nodes, representing the
 ...
 * %GDE2WEATHER_LOCATION_COUNTRY nodes.
 *
 * Return value: (allow-none): a %GDE2WEATHER_LOCATION_WORLD location, or
 * %NULL if Locations.xml could not be found or could not be parsed.
 **/
Gde2WeatherLocation *
gde2weather_location_new_world (gboolean use_regions)
{
    Gde2WeatherParser *parser;
    Gde2WeatherLocation *world;

    parser = gde2weather_parser_new (use_regions);
    if (!parser)
	return NULL;

    world = location_new_from_xml (parser, GDE2WEATHER_LOCATION_WORLD, NULL);

    gde2weather_parser_free (parser);
    return world;
}

/**
 * gde2weather_location_ref:
 * @loc: a #Gde2WeatherLocation
 *
 ...
 **/
Gde2WeatherLocation *
gde2weather_location_ref (Gde2WeatherLocation *loc)
{
 ...
 **/
void
gde2weather_location_unref (Gde2WeatherLocation *loc)
{
    int i;
	...
	for (i = 0; loc->children[i]; i++) {
	    ...
	    gde2weather_location_unref (loc->children[i]);
	}
	g_free (loc->children);
    }
	...
	for (i = 0; loc->zones[i]; i++)
	    gde2weather_timezone_unref (loc->zones[i]);
	g_free (loc->zones);
    }

    g_slice_free (Gde2WeatherLocation, loc);
}

GType
gde2weather_location_get_type (void)
{
    static volatile gsize type_volatile = 0;

    if (g_once_init_enter (&type_volatile)) {
	GType type = g_boxed_type_register_static (
	    g_intern_static_string ("Gde2WeatherLocation"),
	    (GBoxedCopyFunc) gde2weather_location_ref,
	    (GBoxedFreeFunc) gde2weather_location_unref);
...
/**
 * gde2weather_location_get_name:
 * @loc: a #Gde2WeatherLocation
 *
 * Gets @loc's name, localized into the current language.
 *
 * Note that %GDE2WEATHER_LOCATION_WEATHER_STATION nodes are not
 ...
 **/
const char *
gde2weather_location_get_name (Gde2WeatherLocation *loc)
{
    g_return_val_if_fail (loc != NULL, NULL);
    return loc->name;
}
...
/**
 * gde2weather_location_get_sort_name:
 * @loc: a #Gde2WeatherLocation
 *
 ...
 **/
const char *
gde2weather_location_get_sort_name (Gde2WeatherLocation *loc)
{
    g_return_val_if_fail (loc != NULL, NULL);
...
/**
 * gde2weather_location_get_level:
 * @loc: a #Gde2WeatherLocation
 *
 * Gets @loc's level, from %GDE2WEATHER_LOCATION_WORLD, to
 * %GDE2WEATHER_LOCATION_WEATHER_STATION.
 *
 * Return value: @loc's level
 **/
Gde2WeatherLocationLevel
gde2weather_location_get_level (Gde2WeatherLocation *loc)
{
    g_return_val_if_fail (loc != NULL, GDE2WEATHER_LOCATION_WORLD);
    return loc->level;
}

/**
 * gde2weather_location_get_parent:
 * @loc: a #Gde2WeatherLocation
 *
 * Gets @loc's parent location.
 *
 * Return value: (transfer none) (allow-none): @loc's parent, or %NULL
 * if @loc is a %GDE2WEATHER_LOCATION_WORLD node.
 **/
Gde2WeatherLocation *
gde2weather_location_get_parent (Gde2WeatherLocation *loc)
{
    g_return_val_if_fail (loc != NULL, NULL);
    return loc->parent;
}
...
/**
 * gde2weather_location_get_children:
 * @loc: a #Gde2WeatherLocation
 *
 ...
 **/
Gde2WeatherLocation **
gde2weather_location_get_children (Gde2WeatherLocation *loc)
{
    static Gde2WeatherLocation *no_children = NULL;
 ...
 **/
void
gde2weather_location_free_children (Gde2WeatherLocation  *loc,
...
/**
 * gde2weather_location_has_coords:
 * @loc: a #Gde2WeatherLocation
 *
 ...
 **/
gboolean
gde2weather_location_has_coords (Gde2WeatherLocation *loc)
{
...
/**
 * gde2weather_location_get_coords:
 * @loc: a #Gde2WeatherLocation
 ...
 * gde2weather_location_has_coords() before calling this.
 **/
void
gde2weather_location_get_coords (Gde2WeatherLocation *loc,
			      double *latitude, double *longitude)
{
...
/**
 * gde2weather_location_get_distance:
 * @loc: a #Gde2WeatherLocation
 * @loc2: a second #Gde2WeatherLocation
 *
 ...
 **/
double
gde2weather_location_get_distance (Gde2WeatherLocation *loc, Gde2WeatherLocation *loc2)
{
...
/**
 * gde2weather_location_get_country:
 * @loc: a #Gde2WeatherLocation
 *
 ...
 **/
const char *
gde2weather_location_get_country (Gde2WeatherLocation *loc)
{
    g_return_val_if_fail (loc != NULL, NULL);
...
/**
 * gde2weather_location_get_timezone:
 * @loc: a #Gde2WeatherLocation
 *
 ...
 **/
Gde2WeatherTimezone *
gde2weather_location_get_timezone (Gde2WeatherLocation *loc)
{
    const char *tz_hint;
    int i;
	...
	if (!loc)
	    return NULL;
	for (i = 0; loc->zones[i]; i++) {
	    if (!strcmp (tz_hint, gde2weather_timezone_get_tzid (loc->zones[i])))
		return loc->zones[i];
	}
	loc = loc->parent;
    }
...
static void
add_timezones (Gde2WeatherLocation *loc, GPtrArray *zones)
{
    int i;

    if (loc->zones) {
	for (i = 0; loc->zones[i]; i++)
	    g_ptr_array_add (zones, gde2weather_timezone_ref (loc->zones[i]));
    }
    if (loc->level < GDE2WEATHER_LOCATION_COUNTRY && loc->children) {
	for (i = 0; loc->children[i]; i++)
...
/**
 * gde2weather_location_get_timezones:
 * @loc: a #Gde2WeatherLocation
 *
 * Gets an array of all timezones associated with any location under
 * @loc. You can use gde2weather_location_free_timezones() to free this
 * array.
 *
 ...
 **/
Gde2WeatherTimezone **
gde2weather_location_get_timezones (Gde2WeatherLocation *loc)
{
    GPtrArray *zones;
    ...
    return (Gde2WeatherTimezone **)g_ptr_array_free (zones, FALSE);
}

/**
 * gde2weather_location_free_timezones:
 * @loc: a #Gde2WeatherLocation
 * @zones: an array returned from gde2weather_location_get_timezones()
 *
 * Frees the array of timezones returned by
 * gde2weather_location_get_timezones().
 **/
void
gde2weather_location_free_timezones (Gde2WeatherLocation  *loc,
				  Gde2WeatherTimezone **zones)
{
    int i;
    ...
    for (i = 0; zones[i]; i++)
	gde2weather_timezone_unref (zones[i]);
    g_free (zones);
}

/**
 * gde2weather_location_get_code:
 * @loc: a #Gde2WeatherLocation
 *
 * Gets the METAR station code associated with a
 * %GDE2WEATHER_LOCATION_WEATHER_STATION location.
 *
 * Return value: (allow-none): @loc's METAR station code, or %NULL
 **/
const char *
gde2weather_location_get_code (Gde2WeatherLocation *loc)
{
    g_return_val_if_fail (loc != NULL, NULL);
...
/**
 * gde2weather_location_get_city_name:
 * @loc: a #Gde2WeatherLocation
 *
 * For a %GDE2WEATHER_LOCATION_CITY location, this is equivalent to
 * gde2weather_location_get_name(). For a
 * %GDE2WEATHER_LOCATION_WEATHER_STATION location, it is equivalent to
 * calling gde2weather_location_get_name() on the location's parent. For
 * other locations it will return %NULL.
 *
 * Return value: (allow-none) @loc's city name, or %NULL
 **/
char *
gde2weather_location_get_city_name (Gde2WeatherLocation *loc)
{
    g_return_val_if_fail (loc != NULL, NULL);

    if (loc->level == GDE2WEATHER_LOCATION_CITY)
	return g_strdup (loc->name);
    else if (loc->level == GDE2WEATHER_LOCATION_WEATHER_STATION &&
	     loc->parent &&
	     loc->parent->level == GDE2WEATHER_LOCATION_CITY)
...
WeatherLocation *
gde2weather_location_to_weather_location (Gde2WeatherLocation *gloc,
				       const char *name)
{
    const char *code = NULL, *zone = NULL, *radar = NULL, *tz_hint = NULL;
    Gde2WeatherLocation *l;
    ...
    if (gloc->level == GDE2WEATHER_LOCATION_CITY && gloc->children)
	l = gloc->children[0];
    else
	l = gloc;
    ...
    wloc = weather_location_new (name, code, zone, radar, coords,
				 gde2weather_location_get_country (gloc),
				 tz_hint);
    g_free (coords);
    return wloc;
}
 ...
 **/
WeatherInfo *
gde2weather_location_get_weather (Gde2WeatherLocation *loc)
{
    WeatherLocation *wloc;
    WeatherInfo *info;
    ...
    wloc = gde2weather_location_to_weather_location (loc, NULL);
