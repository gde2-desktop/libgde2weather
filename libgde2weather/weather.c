...
static inline void
gde2weather_gettext_init (void)
{
    static gsize gde2weather_gettext_initialized = FALSE;

    if (G_UNLIKELY (g_once_init_enter (&gde2weather_gettext_initialized))) {
        bindtextdomain (GETTEXT_PACKAGE, GDE2LOCALEDIR);
#ifdef HAVE_BIND_TEXTDOMAIN_CODESET
        bind_textdomain_codeset (GETTEXT_PACKAGE, "UTF-8");
#endif
        g_once_init_leave (&gde2weather_gettext_initialized, TRUE);
    }
}

const char *
gde2weather_gettext (const char *str)
{
    gde2weather_gettext_init ();
    return dgettext (GETTEXT_PACKAGE, str);
}

const char *
gde2weather_dpgettext (const char *context,
                    const char *str)
{
    gde2weather_gettext_init ();
 ...
 *    The values are estigde2s of the time of the next new, quarter, full and
 *    three-quarter moons.
 *
