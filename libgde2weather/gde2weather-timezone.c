...
/**
 * SECTION:gde2weather-timezone
 * @Title: Gde2WeatherTimezone
 *
 * A timezone.
 *
 * There are no public methods for creating timezones; they can only
 * be created by calling gde2weather_location_new_world() to parse
 * Locations.xml, and then calling various #Gde2WeatherLocation methods
 * to extract relevant timezones from the location hierarchy.
 */
struct _Gde2WeatherTimezone {
    char *id, *name;
...
static Gde2WeatherTimezone *
parse_timezone (Gde2WeatherParser *parser)
{
    Gde2WeatherTimezone *zone = NULL;
	...
	while (xmlTextReaderNodeType (parser->xml) != XML_READER_TYPE_END_ELEMENT) {
		...
		name = gde2weather_parser_get_localized_value (parser);
	    else {
	...
	zone = g_slice_new0 (Gde2WeatherTimezone);
	zone->ref_count = 1;
	zone->id = g_strdup (id);
...
Gde2WeatherTimezone **
gde2weather_timezones_parse_xml (Gde2WeatherParser *parser)
{
    GPtrArray *zones;
...
error_out:
	...
	gde2weather_timezone_unref (zones->pdata[i]);
    g_ptr_array_free (zones, TRUE);
    return NULL;
}

/**
 * gde2weather_timezone_ref:
 * @zone: a #Gde2WeatherTimezone
 *
 ...
 **/
Gde2WeatherTimezone *
gde2weather_timezone_ref (Gde2WeatherTimezone *zone)
{
...
/**
 * gde2weather_timezone_unref:
 * @zone: a #Gde2WeatherTimezone
 *
 * Subtracts 1 from @zone's reference count and frees it if it reaches 0.
 **/
void
gde2weather_timezone_unref (Gde2WeatherTimezone *zone)
{
	...
	g_free (zone->id);
	g_free (zone->name);
	g_slice_free (Gde2WeatherTimezone, zone);
    }
}

GType
gde2weather_timezone_get_type (void)
{
    static volatile gsize type_volatile = 0;

    if (g_once_init_enter (&type_volatile)) {
	GType type = g_boxed_type_register_static (
	    g_intern_static_string ("Gde2WeatherTimezone"),
	    (GBoxedCopyFunc) gde2weather_timezone_ref,
	    (GBoxedFreeFunc) gde2weather_timezone_unref);
...
/**
 * gde2weather_timezone_get_utc:
 *
 * Gets the UTC timezone.
 *
 * Return value: a #Gde2WeatherTimezone for UTC, or %NULL on error.
 **/
Gde2WeatherTimezone *
gde2weather_timezone_get_utc (void)
{
    Gde2WeatherTimezone *zone = NULL;
 ...
 **/
const char *
gde2weather_timezone_get_name (Gde2WeatherTimezone *zone)
{
    g_return_val_if_fail (zone != NULL, NULL);
    return zone->name;
}
 ...
 **/
const char *
gde2weather_timezone_get_tzid (Gde2WeatherTimezone *zone)
{
    g_return_val_if_fail (zone != NULL, NULL);
    return zone->id;
}
...
/**
 * gde2weather_timezone_get_offset:
 * @zone: a #Gde2WeatherTimezone
 *
 ...
 **/
int
gde2weather_timezone_get_offset (Gde2WeatherTimezone *zone)
{
 ...
 **/
gboolean
gde2weather_timezone_has_dst (Gde2WeatherTimezone *zone)
{
...
/**
 * gde2weather_timezone_get_dst_offset:
 * @zone: a #Gde2WeatherTimezone
 *
 ...
 * gde2weather_timezone_has_dst() returns %TRUE.
 *
 * Return value: @zone's daylight/summer time offset, in minutes
 **/
int
gde2weather_timezone_get_dst_offset (Gde2WeatherTimezone *zone)
{
    g_return_val_if_fail (zone != NULL, 0);
