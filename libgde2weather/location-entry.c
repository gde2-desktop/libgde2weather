 ...
 * @Title: Gde2WeatherLocationEntry
 *
 * A subclass of #GtkEntry that provides autocompletion on
 * #Gde2WeatherLocation<!-- -->s
 */

G_DEFINE_TYPE (Gde2WeatherLocationEntry, gde2weather_location_entry, GTK_TYPE_ENTRY)
...
static void gde2weather_location_entry_build_model (Gde2WeatherLocationEntry *entry,
						 Gde2WeatherLocation *top);
...
enum
{
    GDE2WEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME = 0,
    GDE2WEATHER_LOCATION_ENTRY_COL_LOCATION,
    GDE2WEATHER_LOCATION_ENTRY_COL_COMPARE_NAME,
    GDE2WEATHER_LOCATION_ENTRY_COL_SORT_NAME,
    GDE2WEATHER_LOCATION_ENTRY_NUM_COLUMNS
};
...
static gboolean match_selected (GtkEntryCompletion *completion,
				...
				GtkTreeIter        *iter,
				gpointer            entry);
static void     entry_changed (Gde2WeatherLocationEntry *entry);

static void
gde2weather_location_entry_init (Gde2WeatherLocationEntry *entry)
{
    ...
    gtk_entry_completion_set_text_column (completion, GDE2WEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME);
...
finalize (GObject *object)
{
    ...
    if (entry->location)
	gde2weather_location_unref (entry->location);
    if (entry->top)
	gde2weather_location_unref (entry->top);

    G_OBJECT_CLASS (gde2weather_location_entry_parent_class)->finalize (object);
}

static void
gde2weather_location_entry_class_init (Gde2WeatherLocationEntryClass *location_entry_class)
{
	...
	g_param_spec_pointer ("top",
			      "Top Location",
			      "The Gde2WeatherLocation whose children will be used to fill in the entry",
			      G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
    g_object_class_install_property (
	object_class, PROP_LOCATION,
	g_param_spec_pointer ("location",
			      "Location",
			      "The selected Gde2WeatherLocation",
			      G_PARAM_READWRITE));
}
	      ...
	      const GValue *value, GParamSpec *pspec)
{
    switch (prop_id) {
    case PROP_TOP:
	gde2weather_location_entry_build_model (GDE2WEATHER_LOCATION_ENTRY (object),
					     g_value_get_pointer (value));
	break;
    case PROP_LOCATION:
	gde2weather_location_entry_set_location (GDE2WEATHER_LOCATION_ENTRY (object),
					      g_value_get_pointer (value));
	break;
    default:
...
static void
entry_changed (Gde2WeatherLocationEntry *entry)
{
    entry->custom_text = TRUE;
}

static void
set_location_internal (Gde2WeatherLocationEntry *entry,
		       GtkTreeModel          *model,
		       GtkTreeIter           *iter)
{
	...
	gtk_tree_model_get (model, iter,
			    GDE2WEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME, &name,
			    GDE2WEATHER_LOCATION_ENTRY_COL_LOCATION, &loc,
			    -1);
	entry->location = gde2weather_location_ref (loc);
...
/**
 * gde2weather_location_entry_set_location:
 * @entry: a #Gde2WeatherLocationEntry
 * @loc: (allow-none): a #Gde2WeatherLocation in @entry, or %NULL to
 * clear @entry
 *
 ...
 **/
void
gde2weather_location_entry_set_location (Gde2WeatherLocationEntry *entry,
				      Gde2WeatherLocation      *loc)
{
    GtkEntryCompletion *completion;
    GtkTreeModel *model;
    GtkTreeIter iter;
    Gde2WeatherLocation *cmploc;

    g_return_if_fail (GDE2WEATHER_IS_LOCATION_ENTRY (entry));
    ...
    do {
	gtk_tree_model_get (model, &iter,
			    GDE2WEATHER_LOCATION_ENTRY_COL_LOCATION, &cmploc,
			    -1);
	if (loc == cmploc) {
...
/**
 * gde2weather_location_entry_get_location:
 * @entry: a #Gde2WeatherLocationEntry
 *
 * Gets the location that was set by a previous call to
 * gde2weather_location_entry_set_location() or was selected by the user.
 *
 ...
 **/
Gde2WeatherLocation *
gde2weather_location_entry_get_location (Gde2WeatherLocationEntry *entry)
{
    g_return_val_if_fail (GDE2WEATHER_IS_LOCATION_ENTRY (entry), NULL);

    if (entry->location)
	return gde2weather_location_ref (entry->location);
    else
	return NULL;
}

/**
 * gde2weather_location_entry_has_custom_text:
 * @entry: a #Gde2WeatherLocationEntry
 *
 ...
 * gde2weather_location_entry_get_location() should be used for this.
 *
 * Return value: %TRUE if @entry's text was modified by the user, or %FALSE if
 * it's set to the default text of a location.
 **/
gboolean
gde2weather_location_entry_has_custom_text (Gde2WeatherLocationEntry *entry)
{
    g_return_val_if_fail (GDE2WEATHER_IS_LOCATION_ENTRY (entry), FALSE);

    return entry->custom_text;
}

/**
 * gde2weather_location_entry_set_city:
 * @entry: a #Gde2WeatherLocationEntry
 ...
 **/
gboolean
gde2weather_location_entry_set_city (Gde2WeatherLocationEntry *entry,
				  ...
				  const char            *code)
{
	...
	cmpcode = gde2weather_location_get_code (cmploc);
	if (!cmpcode || strcmp (cmpcode, code) != 0)
	    continue;

	if (city_name) {
	    cmpname = gde2weather_location_get_city_name (cmploc);
...
static void
fill_location_entry_model (GtkTreeStore *store, Gde2WeatherLocation *loc,
			   ...
			   const char *parent_compare_name)
{
    ...
    switch (gde2weather_location_get_level (loc)) {
	...
	for (i = 0; children[i]; i++) {
	    fill_location_entry_model (store, children[i],
				       gde2weather_location_get_name (loc),
				       gde2weather_location_get_sort_name (loc));
	}
	break;

    case GDE2WEATHER_LOCATION_ADM1:
	/* Recurse, adding the ADM1 name to the country name */
	display_name = g_strdup_printf ("%s, %s", gde2weather_location_get_name (loc), parent_display_name);
	compare_name = g_strdup_printf ("%s, %s", gde2weather_location_get_sort_name (loc), parent_compare_name);
	...
	if (children[0] && children[1]) {
		...
		display_name = g_strdup_printf ("%s (%s), %s",
						gde2weather_location_get_name (loc),
						gde2weather_location_get_name (children[i]),
						parent_display_name);
		compare_name = g_strdup_printf ("%s (%s), %s",
						gde2weather_location_get_sort_name (loc),
						gde2weather_location_get_sort_name (children[i]),
						parent_compare_name);

		gtk_tree_store_append (store, &iter, NULL);
		gtk_tree_store_set (store, &iter,
				    GDE2WEATHER_LOCATION_ENTRY_COL_LOCATION, children[i],
				    GDE2WEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME, display_name,
				    GDE2WEATHER_LOCATION_ENTRY_COL_COMPARE_NAME, compare_name,
				    -1);
	    ...
	    gtk_tree_store_append (store, &iter, NULL);
	    gtk_tree_store_set (store, &iter,
				GDE2WEATHER_LOCATION_ENTRY_COL_LOCATION, children[0],
				GDE2WEATHER_LOCATION_ENTRY_COL_DISPLAY_NAME, display_name,
				GDE2WEATHER_LOCATION_ENTRY_COL_COMPARE_NAME, compare_name,
				-1);
...
static void
gde2weather_location_entry_build_model (Gde2WeatherLocationEntry *entry,
				     Gde2WeatherLocation *top)
{
    GtkTreeStore *store = NULL;

    g_return_if_fail (GDE2WEATHER_IS_LOCATION_ENTRY (entry));
    entry->top = gde2weather_location_ref (top);
	 ...
	 GtkTreeIter *iter, gpointer user_data)
{
    ...
    gtk_tree_model_get (gtk_entry_completion_get_model (completion), iter,
			GDE2WEATHER_LOCATION_ENTRY_COL_COMPARE_NAME, &name_mem,
			GDE2WEATHER_LOCATION_ENTRY_COL_LOCATION, &loc,
			-1);
    name = name_mem;
...
/**
 * gde2weather_location_entry_new:
 * @top: the top-level location for the entry.
 *
 * Creates a new #Gde2WeatherLocationEntry.
 *
 * @top will normally be a location returned from
 * gde2weather_location_new_world(), but you can create an entry that
 * only accepts a smaller set of locations if you want.
 *
 * Return value: the new #Gde2WeatherLocationEntry
 **/
GtkWidget *
gde2weather_location_entry_new (Gde2WeatherLocation *top)
{
    return g_object_new (GDE2WEATHER_TYPE_LOCATION_ENTRY,
			 "top", top,
			 NULL);
}
